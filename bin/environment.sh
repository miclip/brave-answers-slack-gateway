...
stack_name=$(prompt_for_value "StackName" "Name for slack bot" "BraveBot" "^[A-Za-z][A-Za-z0-9-]{0,127}$")
# From Bash 3 - testing with MacOS - m and n must be in the range from 0 to RE_DUP_MAX (default 255), inclusive.
region=$(prompt_for_value "AWSRegion" "AWS Region" $(aws configure get region) "us-west-2" "^[a-z]{2}-[a-z]+-[0-9]+$")
